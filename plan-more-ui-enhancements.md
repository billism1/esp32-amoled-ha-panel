...
14. **UE14** — report-row drill-down: tapping a UE12 report row opens a modal
    listing the *member entities behind the number* (the lights that are on, the
    unavailable devices, the sensors in a min/max/avg group). Reuses the same
    filter routine `compute_report_` already runs; members render as **real,
    fully-interactive `make_entity_row` rows** (tap toggles, long-press = detail —
    page parity) via the existing routers. Re-enables the report-row tap UE12 left
    inert (the report row itself still fires no service); closing returns to the
    page. Depends on UE12.
